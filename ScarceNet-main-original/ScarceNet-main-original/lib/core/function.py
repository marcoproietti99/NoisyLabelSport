...
def train(config, train_loader, model, criterion, optimizer, epoch,
          output_dir, tb_log_dir, writer_dict, top_k):
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        #Codice per Curriculum Loss

        if isinstance(outputs, list):
            loss = criterion(outputs[0], target, target_weight, top_k)
            for output in outputs[1:]:
                loss += criterion(output, target, target_weight, top_k)
        else:
            output = outputs
            loss = criterion(output, target, target_weight, top_k)

        print(f"Epoch: {epoch}, Batch: {i}, Loss: {loss.item()}, Top_k: {top_k}")
        # compute gradient and do update step
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            if animalpose:
                ...
                #print(f"all_boxes before update: {all_boxes[idx:idx + num_images]}")
                all_boxes[idx:idx + num_images, 6] = bbox_ids
                #print(f"all_boxes after update: {all_boxes[idx:idx + num_images]}")
