...
ap10k2coco = np.array([2, 0, 1, 3, 4, 5, 8, 6, 9, 7, 10, 11, 12, 13])
class AnimalAP10KDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def __init__(self,
        ...
        if image_set == 'train':
            ...
            ann_file = self.root + 'annotations/sport-train-split_jitter_sigmadefault_joints6_perturbed_80percent.json'
        elif image_set == 'val':
            logger.info('Loading validation annotations')
            ann_file = self.root + 'annotations/sport-val-split.json'
        else:
            logger.info('Loading testing annotations')
            ann_file = self.root + 'annotations/sport-test-split.json'


        self.coco = COCO(ann_file)
        self.img_ids = self.coco.getImgIds()
        print(f"Loaded image IDs: {self.img_ids}")
        # Scrivi gli ID delle immagini nel file di log
        with open("debug_log.txt", "a") as f:
            f.write(f"Loaded image IDs (dataset): {self.img_ids}\n")
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        for idx, kpt in enumerate(preds):
            ...
            #image_name = f"{image_name}"  # Aggiungi il prefisso
            #if image_name not in self.name2id:
                #raise KeyError(f"Image name not found: {image_name}")
            #image_name = img_path[idx]
            #image_name = img_path[idx][-16:]
            image_id = self.name2id[image_name]
            bbox_id = 0 #Ho solo un bbox per immagine
            #bbox_id = int(all_boxes[idx][6])
            #print(f"image_id: {image_id}, bbox_id: {bbox_id}, total bboxes: {len(self.id2Cat[image_id])}")

            #if bbox_id >= len(self.id2Cat[image_id]):
                #raise IndexError(f"bbox_id {bbox_id} is out of range for image_id {image_id}. "
                                #f"Available bounding boxes: {len(self.id2Cat[image_id])}")
            ...
            _kpts.append({
                ...
                'image': image_id,  # Usa l'indice come identificatore numerico dato che è un intero
                #'image': int(img_path[idx]), # questo se non ho un numero come img_id
    ...
    def _write_coco_keypoint_results(self, keypoints, res_file):
        ...
        with open("debug_log.txt", "a") as f:
            f.write(f"Results being written to {res_file}: {results[:5]}\n")  # Mostra i primi 5 risultati

        logger.info('=> writing results json to %s' % res_file)
        with open(res_file, 'w') as f:
            json.dump(results, f, sort_keys=True, indent=4)
        print(f"Results being written to {res_file}: {results[:5]}")  # Mostra i primi 5 risultati
    ...
    def _do_python_keypoint_eval(self, res_file, res_folder):
        ...
        print(f"COCO image IDs (dataset): {self.img_ids}")
        coco_dt = self.coco.loadRes(res_file)

        # Verifica gli ID delle immagini nei risultati
        result_img_ids = [ann['image_id'] for ann in coco_dt.dataset['annotations']]
        print(f"Image IDs in results: {result_img_ids}")

         # Scrivi i dati nel file di log
        with open("debug_log.txt", "a") as f:
            f.write(f"COCO image IDs (dataset): {self.img_ids}\n")
            f.write(f"Image IDs in results: {result_img_ids}\n")

        # Confronta gli ID
        missing_ids = set(result_img_ids) - set(self.img_ids)
        if missing_ids:
            print(f"Missing image IDs: {missing_ids}")
            with open("debug_log.txt", "a") as f:
                f.write(f"Missing image IDs: {missing_ids}\n")

        extra_ids = set(self.img_ids) - set(result_img_ids)
        if extra_ids:
            print(f"Extra image IDs in dataset: {extra_ids}")
