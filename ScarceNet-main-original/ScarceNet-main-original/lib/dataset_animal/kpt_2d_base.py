...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(image_file): #added by Lucia
            logger.error(f"=> Image file not found: {image_file}") #added by Lucia
            raise FileNotFoundError(f"Image file not found: {image_file}") #added by Lucia
        '''

        image_file = results['image_file']
        #image_file = image_file.replace('\\', '/')  # <--- AGGIUNGI QUESTA RIGA
        filename = results['filename'] if 'filename' in results else ''
        imgnum = results['imgnum'] if 'imgnum' in results else ''
        ...
        if data_numpy is None: #added by Lucia
            logger.error(f"=> Fail to read image file: {image_file}") #added by Lucia
            raise ValueError(f"Fail to read image file: {image_file}") #added by Lucia
